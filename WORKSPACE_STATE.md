...
- **工作目錄狀態**：有未 commit 的變更（AI 專案地圖機制，見下）

## ✅ 已完成（最近）

以 **最新 commit 為準**，開發脈絡（由新到舊）：

1. 提貨時間選擇器：自訂 ±15 分鐘跨小時進位、CLOCK 彈出 00/15/30/45、stopPropagation 修正
2. 訂單系統重大更新：6 項新功能 + 後端重構 + 多類別支援
3. MAWB# 驗證／後補、重複檢查、AGL 流水號
4. 電力分類改為累積新增模式（可混合無電/乾電/鋰電，各自輸入件數）
5. 資料庫檢視器新增（白名單保護、外鍵下拉、刪除關聯保護）
...
- [ ] 把地圖機制 commit（建議 message：`新增 AI 專案地圖與狀態同步機制`）
