...
- **最後 commit**：`c3b83ab` 修正提貨時間選擇器：點選時/分後不會誤關閉彈出視窗（stopPropagation）
- **目前分支**：master（github.com/BetataLAU/AGL-Web-Portal）
- **工作目錄狀態**：有未 commit 的變更（訂單系統調整 + 通用工具抽取，見下）

## ✅ 已完成（最近）

以 **最新 commit 為準**，開發脈絡（由新到舊）：

1. **通用工具抽取**：從 `orders.js` 抽出 6 個可重用工具到 `public/js/utils/`（api / datetime / mawb / modal / cbm-calculator / time-picker），CSS 對應搬至 `public/css/utils/`，`index.html` 已在 orders.js 之前引入
2. **訂單系統調整**：範本功能移除（UI + API，資料庫 templates 表保留）、收貨/送貨按鈕對調、HAWB# 改非必填、第 8️⃣ 區塊改為只有備註（運輸公司選擇移除）
3. 提貨時間選擇器：自訂 ±15 分鐘跨小時進位、CLOCK 彈出 00/15/30/45、stopPropagation 修正
4. 訂單系統重大更新：6 項新功能 + 後端重構 + 多類別支援
5. MAWB# 驗證／後補、重複檢查、AGL 流水號
6. 電力分類改為累積新增模式（可混合無電/乾電/鋰電，各自輸入件數）
7. 資料庫檢視器新增（白名單保護、外鍵下拉、刪除關聯保護）
